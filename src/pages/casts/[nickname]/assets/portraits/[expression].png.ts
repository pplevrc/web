...
> = async ({ params }) => {
  const { nickname, expression } = params;

  try {
    const casts = await fetchCasts();
    const cast = casts.find((c) => c.profile.nickname === nickname);

    if (!cast) {
      return new Response("Cast not found", { status: 404 });
    }

    const src = cast.portrait[expression as keyof AvatarImages];
    ...
    console.error("Error fetching portrait image:", error);
